...
const addTask = (task) => {
	const column = columns.value.find(col => col.name === task.column);
	if (column) {
		column.tasks.push({
			id: `t${Date.now()}`,
			title: task.title,
...
<template>
	 <div>
		<Header :title="title" @open-modal="openModal"></Header>
		<Board :columns="columns" @delete="deleteTask"></Board>
		<Modal v-show="showModal" :tags="tags" :columns="columns" @close="closeModal" @submit="addTask"/>
	</div>
</template>